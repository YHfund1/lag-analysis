...
<body>
  ...
  <script>
    ...
    function drawChart(data, factorLabel, lag) {{
      ...
      const margin = {{ top: 28, right: 78, bottom: 48, left: 62 }};
      ...
      addText(margin.left, 18, '10年国债收益率 %', '#111827', 'start', 14, 700);
      addText(width - margin.right, 18, `${{factorLabel}}，领先 ${{lag}} 期`, '#111827', 'end', 14, 700);
    ...
    function drawGrid(width, height, margin, plotW, plotH, y1Domain, y1Scale, y2Domain, y2Scale, xMin, xMax, xScale) {{
      ...
      for (let i = 0; i <= 5; i++) {{
        ...
        addText(margin.left - 10, y + 4, fmt.format(yValue), '#111827', 'end', 13, 700);
        const rightValue = y2Domain[0] + (i / 5) * (y2Domain[1] - y2Domain[0]);
        addText(width - margin.right + 10, y + 4, compactNumber(rightValue), '#111827', 'start', 13, 700);
      }}
      for (let i = 0; i <= 6; i++) {{
        const xValue = xMin + (i / 6) * (xMax - xMin);
        const x = xScale(xValue);
        line(x, margin.top, x, margin.top + plotH, '#eef1f5', 1, gridGroup);
        addText(x, height - 18, dateFmt.format(new Date(xValue)), '#111827', 'middle', 13, 700);
      }}
