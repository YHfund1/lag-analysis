...
<body>
  ...
  <script>
    ...
    function drawChart(data, factorLabel, lag) {{
      ...
      addText(margin.left, 18, '10年国债收益率 %', '#2563eb', 'start', 12, 600);
      addText(width - margin.right, 18, `${{factorLabel}}，领先 ${{lag}} 期`, '#0f9f6e', 'end', 12, 600);
    ...
    function drawGrid(width, height, margin, plotW, plotH, y1Domain, y1Scale, y2Domain, y2Scale, xMin, xMax, xScale) {{
      ...
      for (let i = 0; i <= 5; i++) {{
        ...
        addText(margin.left - 10, y + 4, fmt.format(yValue), '#667085', 'end', 11, 400);
        const rightValue = y2Domain[0] + (i / 5) * (y2Domain[1] - y2Domain[0]);
        addText(width - margin.right + 10, y + 4, compactNumber(rightValue), '#667085', 'start', 11, 400);
      }}
      for (let i = 0; i <= 6; i++) {{
        const xValue = xMin + (i / 6) * (xMax - xMin);
        const x = xScale(xValue);
        line(x, margin.top, x, margin.top + plotH, '#eef1f5', 1, gridGroup);
        addText(x, height - 18, dateFmt.format(new Date(xValue)), '#667085', 'middle', 11, 400);
      }}
